...
def visualize_classification_results(bands, indices, classification, method_name="Classification"):
    ...
    print(f"Classification dtype: {classification.dtype}")
    ...
    axes[0, 1].axis('off')
    plt.colorbar(ndvi_plot, ax=axes[0, 1], fraction=0.046, shrink=0.8)
    
    # NDWI
    ndwi_plot = axes[0, 2].imshow(indices['ndwi'], cmap='Blues', vmin=-1, vmax=1)
    axes[0, 2].set_title('NDWI')
    axes[0, 2].axis('off')
    plt.colorbar(ndwi_plot, ax=axes[0, 2], fraction=0.046, shrink=0.8)
    
    # Classification result - Fixed colormap
    from matplotlib.colors import ListedColormap
    class_colors = ['#000000', '#0066CC', '#66CC00', '#CC6600', '#006600']  # Better colors
    class_cmap = ListedColormap(class_colors)
    
    # Ensure classification is properly displayed
    classification_display = classification.astype(float)
    class_plot = axes[1, 0].imshow(classification_display, cmap=class_cmap, vmin=0, vmax=4, interpolation='nearest')
    axes[1, 0].set_title('Land Cover Classification')
    axes[1, 0].axis('off')
    
    # Create custom colorbar
    cbar = plt.colorbar(class_plot, ax=axes[1, 0], fraction=0.046, shrink=0.8)
    ...
    # Create overlay mask
    overlay = np.ma.masked_where(classification == 0, classification)
    axes[1, 1].imshow(overlay, alpha=0.7, cmap=class_cmap, vmin=0, vmax=4, interpolation='nearest')
    ...
    colors = ['#000000', '#0066CC', '#66CC00', '#CC6600', '#006600']
    
    # Only plot classes that exist in the data
    plot_colors = [colors[i] for i in unique]
    plot_labels = [class_names[i] for i in unique]
    
    bars = axes[1, 2].bar(range(len(unique)), counts, color=plot_colors)
    axes[1, 2].set_xticks(range(len(unique)))
    axes[1, 2].set_xticklabels(plot_labels, rotation=45, ha='right')
    axes[1, 2].set_title('Class Distribution')
    axes[1, 2].set_ylabel('Pixel Count')
    
    # Add value labels on bars
    for i, (bar, count) in enumerate(zip(bars, counts)):
        axes[1, 2].text(bar.get_x() + bar.get_width()/2, bar.get_height() + max(counts)*0.01,
                       f'{count:,}', ha='center', va='bottom', fontsize=9)
    
    plt.tight_layout()
    plt.show()
    
    # Also create a standalone classification plot
    plt.figure(figsize=(10, 8))
    plt.imshow(classification_display, cmap=class_cmap, vmin=0, vmax=4, interpolation='nearest')
    plt.title(f'{method_name} - Land Cover Classes', fontsize=14)
    cbar = plt.colorbar(fraction=0.046, shrink=0.8)
    cbar.set_ticks([0, 1, 2, 3, 4])
    cbar.set_ticklabels(['Unclassified', 'Water', 'Vegetation', 'Built-up/Bare', 'Dense Veg'])
    plt.axis('off')
    plt.tight_layout()
